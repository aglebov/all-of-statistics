kreg <- function(x, y, bandwidth, x.points) {
  bw <- bandwidth * 4 * qnorm(0.75)
  return(ksmooth(x, y, bandwidth=bw, x.points=x.points, kernel="normal"))
}
...
cv.exact.fun <- function(h,x,y){
     ...
     for(j in 1:nh){
          temp <- rep(0,n)
          for(i in 1:n){
               junk    <- kreg(x[-i], y[-i], bandwidth=h[j], x.points=x[i])$y
               temp[i] <- (junk - y[i])^2
               }
          risk[j] <- sum(temp,na.rm=T)
          }
...
cv.approx.fun <- function(h,x,y){
     ...
     risk <- rep(0,nh)
     for(j in 1:nh){
          f <- kreg(x, y, bandwidth=h[j], x.points=x)$y
          w <- rep(0,n)
          for(i in 1:n){
...
par(mfrow=c(1,1))
temp  <- cv.exact.fun(h,x,y)
plot(h,temp$risk,type="l",lwd=3,xlab="bandwidth",ylab="CV score",col="green")
temp  <- cv.approx.fun(h,x,y)
lines(h,temp$risk,type="l",lwd=3,lty=2, col="blue")
#dev.off()
...
f <- kreg(x, y, bandwidth=h.opt, x.points=grid)$y
sd    <- rep(0,k)
ess   <- sd
for(i in 1:k){
     w <- dnorm(grid[i],x,h)
...
U <- rep(u,rep(2,k))
L <- rep(l,rep(2,k))
plot(grid,u,type="l",lwd=4,xlab="",ylab="",ylim=c(-1500,6000))
